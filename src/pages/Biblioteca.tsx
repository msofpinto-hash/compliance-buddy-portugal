...
const themeConfig: Record<string, { 
 ...
 ring: "ring-orange-300/40 dark:ring-orange-500/30"},"Segurança e Saúde no Trabalho": { 
 icon: Shield, 
 color: "text-orange-600 dark:text-orange-400", 
 bgLight: "bg-orange-100", 
 bgDark: "dark:bg-orange-900/40", 
 ...
 gradient: "from-orange-500 to-red-600",
 gradientDark: "dark:from-orange-400 dark:to-red-500",
 ring: "ring-orange-300/40 dark:ring-orange-500/30"},"Energia": { 
 icon: Zap, 
 color: "text-primary ", 
 bgLight: "bg-accent", 
 bgDark: "", 
 ...
 ring: "ring-primary/40 "},"Qualidade": { 
 icon: Award, 
 ...
 ring: "ring-sky-300/40 dark:ring-sky-500/30"},"Segurança": { 
 icon: Shield, 
 ...
 ring: "ring-rose-300/40 dark:ring-rose-500/30"},"Conciliação Familiar e Profissional": { 
 icon: Heart, 
...
export default function Biblioteca() {
 ...
 badge="Gestão Documental"icon={BookOpen}
 stats={[
 ...
 <TabsTrigger value="all" className="text-xs data-[state=active]:bg-primary data-[state=active]:text-white dark:data-[state=active]:bg-primary">
 Todos
 </TabsTrigger>
 <TabsTrigger value="dre" className="text-xs gap-1 data-[state=active]:bg-primary data-[state=active]:text-white dark:data-[state=active]:bg-primary">
 <Flag className="h-3 w-3" />
 PT
 </TabsTrigger>
 <TabsTrigger value="eurlex" className="text-xs gap-1 data-[state=active]:bg-primary data-[state=active]:text-white dark:data-[state=active]:bg-primary">
 ...
 !selectedThemeId 
 ? "bg-gradient-to-br from-primary to-primary text-white shadow-md ring-2 ring-primary/30 ":"bg-muted border border-border/60 hover:bg-accent text-muted-foreground ")}
 ...
 <LayoutGrid className={cn("h-5 w-5", !selectedThemeId ? "text-white":"text-primary ")} />
 </div>
 ...
 >
 <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center",
 isSelected ? "bg-white/20" : `bg-white ${config.border}`
 )}>
 ...
 {selectedSource !== "all" && (
 <Badge variant="secondary" className="gap-1 bg-accent text-primary border-0">
 {selectedSource === "dre" ? <Flag className="h-3 w-3" /> : <Globe className="h-3 w-3"/>}
 {selectedSource ==="dre"?"Portugal":"UE"}
 <button onClick={() => setSelectedSource("all")} className="ml-1 hover:bg-accent rounded-full p-0.5">
 <X className="h-3 w-3" />
 </button>
 </Badge>
 )}
 {selectedThemeId && themes && (
 <Badge variant="secondary" className="gap-1 bg-primary text-primary border-0">
 <Tags className="h-3 w-3" />
 {themes.find(t => t.id === selectedThemeId)?.name}
 <button onClick={() => { setSelectedThemeId(null); setSelectedCategoryId(null); }} className="ml-1 hover:bg-primary rounded-full p-0.5">
 <X className="h-3 w-3" />
 </button>
 </Badge>
 )}
 ...
 {searchTerm && (
 <Badge variant="secondary" className="gap-1 bg-muted text-foreground border-0">
 ...
 {[1, 2, 3, 4, 5].map((i) => (
 <Skeleton key={i} className="h-32 w-full rounded-xl bg-accent/50 " />
 ))}
 </div>
 ...
 <motion.div 
 className="p-6 rounded-full bg-gradient-to-br from-primary to-primary mb-6"initial={{ scale: 0 }}
 animate={{ scale: 1 }}
 transition={{ type:"spring", stiffness: 200 }}
 >
 <FileText className="h-12 w-12 text-primary " />
 </motion.div>
 <h3 className="text-xl font-semibold mb-2 text-foreground dark:text-white">Nenhum diploma encontrado</h3>
