...
const themeConfig: Record<string, { icon: React.ElementType; color: string; bgLight: string; bgDark: string; border: string }> = {
 "Ambiente": { icon: Leaf, color: "text-primary", bgLight: "bg-primary", bgDark: "", border: "border-primary "},"SST": { icon: Shield, color: "text-orange-600", bgLight: "bg-orange-100", bgDark: "dark:bg-orange-900/40", border: "border-orange-300 dark:border-orange-700"},"Segurança e Saúde no Trabalho": { icon: Shield, color: "text-orange-600", bgLight: "bg-orange-100", bgDark: "dark:bg-orange-900/40", border: "border-orange-300 dark:border-orange-700"},"Energia": { icon: Zap, color: "text-yellow-600", bgLight: "bg-yellow-100", bgDark: "dark:bg-yellow-900/40", border: "border-yellow-300 dark:border-yellow-700"},"Qualidade": { icon: Award, color: "text-blue-600", bgLight: "bg-blue-100", bgDark: "dark:bg-blue-900/40", border: "border-blue-300 dark:border-blue-700"},"Segurança": { icon: Shield, color: "text-red-600", bgLight: "bg-red-100", bgDark: "dark:bg-red-900/40", border: "border-red-300 dark:border-red-700"},"Conciliação Familiar e Profissional": { icon: Heart, color: "text-pink-600", bgLight: "bg-pink-100", bgDark: "dark:bg-pink-900/40", border: "border-pink-300 dark:border-pink-700" },
};
...
export function BibliotecaContent({ organizationId }: BibliotecaContentProps) {
 ...
 {/* Header */}
 <div>
 <h2 className="text-2xl font-bold bg-gradient-to-r from-primary to-teal-400 bg-clip-text text-transparent flex items-center gap-3">
 ...
 <TabsTrigger value="dre" className="text-xs gap-1 data-[state=active]:bg-primary/20 data-[state=active]:text-primary">
 <Flag className="h-3 w-3" />
 PT
 </TabsTrigger>
 <TabsTrigger value="eurlex" className="text-xs gap-1 data-[state=active]:bg-primary/20 data-[state=active]:text-primary">
 ...
 !selectedThemeId 
 ? "bg-gradient-to-br from-primary/20 to-teal-500/20 border-2 border-primary/50 shadow-md":"bg-card/50 border-2 border-transparent hover:border-primary/30 hover:bg-primary/10")}
 whileHover={{ scale: 1.02 }}
 whileTap={{ scale: 0.98 }}
 >
 <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center",
 !selectedThemeId ? "bg-gradient-to-br from-primary to-teal-500 text-white shadow-lg":"bg-muted text-muted-foreground"
 )}>
 ...
 className={cn("flex flex-col items-center gap-2 p-3 rounded-xl transition-all duration-200 min-w-[100px] shrink-0 relative overflow-hidden",
 isSelected 
 ...
 <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center relative z-10 shadow-sm",
 ...
 <span className="font-semibold text-primary">{filteredCount}</span> diploma{filteredCount !== 1 ? "s":""} encontrado{filteredCount !== 1 ? "s":""}
 </p>
 ...
 <button onClick={() => { setSelectedThemeId(null); setSelectedCategoryId(null); }} className="ml-1 hover:bg-primary/30 rounded-full p-0.5">
 <X className="h-3 w-3" />
 </button>
 </Badge>
 )}
 ...
 <Search className="h-3 w-3"/>"{searchTerm.slice(0, 20)}{searchTerm.length > 20 ? '...' : ''}"
 ...
 <Card className="py-20 bg-card/60 backdrop-blur-xl border-primary/20">
 <CardContent className="flex flex-col items-center justify-center text-center">
 <motion.div 
 className="p-6 rounded-full bg-primary/10 mb-6"initial={{ scale: 0 }}
 animate={{ scale: 1 }}
 ...
 <Button
 variant="outline"
 className="mt-6 gap-2 border-primary/30 text-primary hover:bg-primary/10"
 onClick={clearAllFilters}
 >
 <X className="h-4 w-4" />
 Limpar filtros
