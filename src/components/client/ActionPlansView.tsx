...
> = {
  ...
  em_curso: {
    label: "Em Curso",
    color: "text-primary",
    bgColor: "bg-accent border-primary",
    icon: AlertCircle,
...
> = {
  alta: {
    ...
    dotColor: "bg-gradient-to-br from-rose-400 to-rose-600",
  ...
  media: {
    ...
    color: "text-primary",
    bgColor: "bg-accent border-primary",
    dotColor: "bg-gradient-to-br from-primary to-primary",
...
function ImportFromAuditDialog({
 ...
 `,
  ...
  const availableFindings =
    findings?.filter((f) => !existingPlans?.includes(f.id)) || [];

  const handleImport = async () => {
    if (selectedFindings.length === 0) {
      toast({
        title: "Selecione pelo menos uma constatação",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const plansToCreate = selectedFindings.map((findingId) => {
        ...
        return {
          ...
          description:
            (finding as any)?.findings ||
            (finding as any)?.legal_requirements?.requirement_text ||
            "",
          status: "pendente",
          created_by: user?.id,
        };
      ...
      toast({
        ...
        description: `${plansToCreate.length} planos importados`,
      });
      setOpen(false);
      setSelectedFindings([]);
      onImported();
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div
              className={`space-y-2 ${organizations.length === 1 ? "col-span-2" : ""}`}
            >
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-muted-foreground">
                              {finding.legislation?.number} -{" "}
                              {finding.legal_requirements?.article}
                            </p>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isImporting || selectedFindings.length === 0}
          >
 ...
 *,
 ...
 `,
  ...
  const uniqueResponsibles = useMemo(() => {
    const responsibles =
      actionPlans?.map((p) => p.responsible).filter(Boolean) || [];
  ...
  const filteredPlans = useMemo(() => {
    ...
    const filtered = actionPlans.filter((plan) => {
      ...
      if (
        ...
        !filters.status.includes(plan.status || "pendente")
      )
        return false;
      ...
      if (
        ...
        (plan.priority || "media") !== filters.priority
      )
        return false;

      // Responsible filter
      if (filters.responsible && plan.responsible !== filters.responsible)
        return false;
      ...
      if (
        filters.dateStart &&
        plan.due_date &&
        new Date(plan.due_date) < new Date(filters.dateStart)
      )
        return false;
      if (
        filters.dateEnd &&
        plan.due_date &&
        new Date(plan.due_date) > new Date(filters.dateEnd)
      )
        return false;
  ...
  const exportToExcel = async () => {
    ...
    const summaryRows = [
      ...
      {
        metrica: "Pendentes",
        valor: filteredPlans.filter((p) => p.status === "pendente").length,
      },
      {
        metrica: "Em Curso",
        valor: filteredPlans.filter((p) => p.status === "em_curso").length,
      },
      {
        metrica: "Concluídas",
        valor: filteredPlans.filter((p) => p.status === "concluido").length,
      },
      {
        metrica: "De Auditoria",
        valor: filteredPlans.filter((p) => p.audit_requirement_id).length,
      },
      {
        metrica: "Ad-hoc",
        valor: filteredPlans.filter((p) => !p.audit_requirement_id).length,
      ...
      {
        metrica: "Data Exportação",
        valor: format(new Date(), "dd/MM/yyyy HH:mm"),
      },
    ];
    ...
    toast({
      ...
      description: `${filteredPlans.length} registos exportados`,
    });
  };
  ...
  const exportToPDF = () => {
    ...
    const pdfStats = [
      ...
      {
        label: "Pendentes",
        value: filteredPlans.filter((p) => p.status === "pendente").length,
        color: [107, 114, 128] as [number, number, number],
      },
      {
        label: "Em Curso",
        value: filteredPlans.filter((p) => p.status === "em_curso").length,
        color: [234, 179, 8] as [number, number, number],
      },
      {
        label: "Concluídas",
        value: filteredPlans.filter((p) => p.status === "concluido").length,
    ...
    pdfStats.forEach((stat, i) => {
      ...
      doc.text(String(stat.value), x + boxWidth / 2, statsY + 10, {
        align: "center",
      });
      doc.setFontSize(7);
      doc.setTextColor(107, 114, 128);
 ...
 ${filteredPlans
   .map((plan) => {
     const typeClass = plan.audit_requirement_id
       ? "print-badge-audit"
       : "print-badge-adhoc";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        <IDCard
          className={`group cursor-pointer transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${!filters.status.length && filters.type === "all" ? "ring-2 ring-primary/30 border-primary/50" : ""}`}
        >
          <CardContent
        ...
        <Card
          className={`group cursor-pointer transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 bg-white dark:bg-slate-800 ${filters.status.includes("em_curso") ? "ring-2 ring-primary border-primary" : ""}`}
          onClick={() => toggleStatusFilter("em_curso")}
        >
          <CardContent className="p-4 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-primary to-transparent opacity-0 group-hover:opacity-100 transition-opacity " />
            <div className="flex items-center justify-center gap-2">
        ...
        <Card
          className={`group cursor-pointer transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 bg-white dark:bg-slate-800 ${filters.status.includes("concluido") ? "ring-2 ring-primary border-primary" : ""}`}
            ...
            <CardContent className="p-5">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="media">
                        <div className="flex items-center gap-2">
                          <span className="h-2.5 w-2.5 rounded-full bg-gradient-to-br from-primary to-primary" />
                          Média
                        </div>
                      </SelectItem>
                      <SelectItem value="baixa">
              ...
              <div className="flex items-center gap-2 mt-5 pt-4 border-t border-dashed">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    setFilters((p) => ({ ...p, hideCompleted: !!v }))
                  }
                />
                <Label
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        {plan.due_date ? (
                          <div
                            className={`flex items-center gap-1.5 ${overdue ? "text-rose-600 font-semibold" : "text-muted-foreground"}`}
                          >
                            <Calendar
                              className={`h-3.5 w-3.5 ${overdue ? "text-rose-500" : ""}`}
                            />
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={`gap-1.5 font-medium ${statusInfo.bgColor} ${statusInfo.color}`}
                        >
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <Select
                          value={plan.status || "pendente"}
                          onValueChange={(v) => handleStatusChange(plan.id, v)}
                        >
              ...
              >
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className={`gap-1.5 font-medium text-xs ${statusInfo.bgColor} ${statusInfo.color}`}
                    >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-dashed">
                    ...
                    {plan.due_date && (
                      <div
                        className={`flex items-center gap-1.5 text-xs ${overdue ? "text-rose-600 font-semibold" : "text-muted-foreground"}`}
                      >
                        <Calendar
                          className={`h-3.5 w-3.5 ${overdue ? "text-rose-500" : ""}`}
                        />
          ...
          <DialogHeader className="pb-4 border-b">
            <div className="flex items-center gap-3">
              {selectedPlan && (
                <Badge
                  variant="outline"
                  className={`text-xs font-medium ${getTypeInfo(selectedPlan).color}`}
                >
                  {getTypeInfo(selectedPlan).icon}{" "}
              ...
              {selectedPlan && isOverdue(selectedPlan) && (
                <Badge
                  variant="outline"
                  className="text-xs font-medium bg-rose-50 text-rose-700 border-rose-200"
                >
                  ⚠️ Em Atraso
                </Badge>
              )}
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="grid grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg">
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`h-3.5 w-3.5 rounded-full shadow-sm ring-2 ${priorityConfig[selectedPlan.priority || "media"].dotColor} ${priorityConfig[selectedPlan.priority || "media"].ringColor}`}
                    />
                    <span
                      className={`font-medium ${priorityConfig[selectedPlan.priority || "media"].color}`}
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={selectedPlan.status || "pendente"}
                    onValueChange={(v) => {
                ...
                <div className="space-y-1.5">
                  ...
                  <div
                    className={`flex items-center gap-1.5 font-medium ${isOverdue(selectedPlan) ? "text-rose-600" : "text-foreground"}`}
                  >
                    <Calendar
                      className={`h-4 w-4 ${isOverdue(selectedPlan) ? "text-rose-500" : "text-muted-foreground"}`}
                    />
                    {selectedPlan.due_date
                      ? format(new Date(selectedPlan.due_date), "dd/MM/yyyy")
                      : "—"}
                  </div>
                </div>
              </div>
              ...
              {selectedPlan.audit_requirements?.audits && (
                <div className="space-y-1.5 p-3 bg-sky-50/50 border border-sky-100 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-sky-900">
                    {selectedPlan.audit_requirements.audits.title}
                  </p>
                </div>
              )}
