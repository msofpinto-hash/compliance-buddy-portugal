...
export const IDBackground = () => (
 ...
 <motion.div
 className="absolute top-1/4 right-1/3 w-2 h-2 rounded-full bg-primary/25 "animate={{
 y: [-20, 20, -20],
 opacity: [0.2, 0.5, 0.2],
 }}
 transition={{
 ...
 />
 <motion.div
 className="absolute bottom-1/3 left-1/4 w-3 h-3 rounded-full bg-primary/20 "animate={{
 y: [15, -15, 15],
 opacity: [0.2, 0.4, 0.2],
 }}
 transition={{
 ...
 />
 <motion.div
 className="absolute top-2/3 right-1/4 w-2 h-2 rounded-full bg-orange-400/20 dark:bg-orange-300/10"animate={{
 y: [-15, 15, -15],
...
export const IDParticles = ({ count = 8 }: { count?: number }) => {
 ...
 <motion.div
 key={i}
 className="absolute w-1 h-1 rounded-full bg-primary/20 "style={{
 left: `${10 + (i * 80 / count)}%`,
...
export const IDHeroSection = ({ 
 ...
 >
 <p className="text-xl font-bold text-foreground dark:text-white leading-none">{stat.value}</p>
...
export const IDCard = ({ 
 ...
 <div className={`bg-white/95 dark:bg-[#181410]/90 border border-border/60 rounded-xl shadow-sm backdrop-blur-sm ${className}`}>
 {children}
 </div>
);
