CREATE OR REPLACE FUNCTION public.enforce_client_applicability_scope()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Permitir atualizações internas disparadas por outro trigger seguro,
  -- nomeadamente a propagação da aplicabilidade do diploma aos requisitos.
  IF pg_trigger_depth() > 1 THEN
    RETURN NEW;
  END IF;

  -- Operações internas sem sessão de utilizador continuam permitidas.
  IF auth.uid() IS NULL THEN
    RETURN NEW;
  END IF;

  -- Administradores podem editar diretamente.
  IF public.has_role(auth.uid(), 'admin'::app_role) THEN
    RETURN NEW;
  END IF;

  -- Clientes não podem alterar campos protegidos dos requisitos.
  NEW.organization_id    := OLD.organization_id;
  NEW.requirement_id     := OLD.requirement_id;
  NEW.is_applicable      := OLD.is_applicable;
  NEW.applicability_type := OLD.applicability_type;
  NEW.compliance_status  := OLD.compliance_status;
  NEW.notes              := OLD.notes;
  NEW.created_at         := OLD.created_at;

  RETURN NEW;
END;
$$;

-- Reconciliar quaisquer requisitos que tenham ficado com o estado anterior
-- devido ao bloqueio do trigger de segurança.
UPDATE public.applicabilities a
SET applicability_type = ol.applicability_type,
    is_applicable = (ol.applicability_type <> 'nao_aplicavel'),
    compliance_status = CASE
      WHEN ol.applicability_type = 'nao_aplicavel' THEN 'nao_aplicavel'
      ELSE a.compliance_status
    END,
    updated_at = now()
FROM public.legal_requirements r
JOIN public.organization_legislation ol
  ON ol.legislation_id = r.legislation_id
WHERE r.id = a.requirement_id
  AND ol.organization_id = a.organization_id
  AND ol.applicability_type IN ('informativo', 'nao_aplicavel')
  AND a.applicability_type IS DISTINCT FROM ol.applicability_type;

INSERT INTO public.applicabilities (
  organization_id,
  requirement_id,
  is_applicable,
  applicability_type,
  compliance_status
)
SELECT ol.organization_id,
       r.id,
       (ol.applicability_type <> 'nao_aplicavel'),
       ol.applicability_type,
       CASE WHEN ol.applicability_type = 'nao_aplicavel' THEN 'nao_aplicavel' ELSE NULL END
FROM public.organization_legislation ol
JOIN public.legal_requirements r
  ON r.legislation_id = ol.legislation_id
WHERE ol.applicability_type IN ('informativo', 'nao_aplicavel')
  AND NOT EXISTS (
    SELECT 1
    FROM public.applicabilities a
    WHERE a.organization_id = ol.organization_id
      AND a.requirement_id = r.id
  );