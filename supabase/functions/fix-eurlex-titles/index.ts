...
async function fetchEURLexTitle(url: string, celex: string, apiKey: string): Promise<string | null> {
  try {
    console.log(`Fetching title for ${celex} from ${url}`);
    ...
    for (const line of lines) {
      ...
      if (/^(Regulamento|Diretiva|Decisão|Recomendação|Decisão de Execução|Regulamento de Execução|Regulamento Delegado)/i.test(trimmed)) {
      ...
      if (trimmed.startsWith('#')) {
        ...
        if (headerText.length > 30 && 
            ...
            /^(Regulamento|Diretiva|Decisão)/i.test(headerText)) {
          title = headerText;
          break;
        }
      }
    ...
    console.error(`Error fetching title for ${celex}:`, error);
    return null;
  }
}
